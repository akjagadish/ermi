...
if __name__ == "__main__":
    ...
    if args.pool:
        ...
        pool_tasklabels(args.path, args.run_gpt, args.model, args.num_dim,
        ...
        if run_gpt == 'llama':
            llama = LLaMAInference(
                args.llama_path, args.model, max_batch_size=2)
        ...
        for run in range(first_run_id, first_run_id+num_runs):
            ...
            if len(matches) > 0:
                for match in matches:
                    # last two labels are categories
                    categories.append(match.split(',')[-2:])
                    # rest are stimulus dimensions
                    stimulus_dimensions.append(match.split(',')[:-2])
                # save data
                df = pd.DataFrame({'feature_names': stimulus_dimensions, 'category_names': categories,
