...
import sys
import os
sys.path.append('/raven/u/ajagadish/ermi/categorisation/')
sys.path.append('/raven/u/ajagadish/ermi/categorisation/data')
    ...
    for model in models:
        for dim in dims:
            for num_data in data:
                ...
                for num_tasks in tasks:
                    for run in runs:
                        for proc_id in proc_ids[num_tasks]:
                            ...
                            last_task_id = parse_generated_tasks(
                                path+'/parsed', filename, gpt, num_data, dim, last_task_id, use_generated_tasklabels, prompt_version)
                ...
                df = df if use_generated_tasklabels else df.query(
...
if __name__ == '__main__':
    ...
    parser.add_argument('--path', type=str, default=None,
                        help='path to the folder containing the generated tasks')
    parser.add_argument('--gpt', type=str, default='claude',
                        help='gpt model used to generate the tasks')
    parser.add_argument('--models', nargs='+',
                        default=['NA'], help='versions of gpt model used to generate the tasks')
    parser.add_argument('--dims', nargs='+', type=int,
                        default=[3], help='list of dimensions used to generate the tasks')
    parser.add_argument('--num-data', nargs='+', type=int,  default=[
                        100], help='list of number of datapoints used to generate the tasks')
    parser.add_argument('--tasks', type=int, default=2000,
                        help='list of number of tasks used to generate the tasks')
    parser.add_argument('--runs', nargs='+', type=int,
                        default=[0], help='list of runs used to generate the tasks')
    parser.add_argument('--proc_ids', nargs='+', type=int,
                        default=[0], help='dictionary of process ids used to generate the tasks')
    parser.add_argument('--prompt_version', type=int, default=4,
                        help='version of the prompt used to generate the tasks')
    parser.add_argument('--use_generated_tasklabels', action='store_true',
                        help='whether to use the task labels generated by the LLMs or the ground truth labels')
    ...
    data = parse_and_pool_generated_tasks(path, gpt, models, dims, num_data_points, [
                                          tasks], runs, proc_ids, prompt_version, use_generated_tasklabels, stage)
