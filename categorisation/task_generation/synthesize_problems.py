...
import sys
sys.path.append("..")
sys.path.insert(1, '/u/ajagadish/ermi/categorisation/')
sys.path.insert(1, '/raven/u/ajagadish/ermi/')
...
if __name__ == "__main__":
    ...
    parser.add_argument("--task", type=str, required=False, default='functionlearning')
    ...
    parser.add_argument("--path", type=str, required=False, default='/u/ajagadish/ermi/categorisation/data/synthesize_problems')
    ...
    if args.pool:
        ...
        # load Claude specific instructions
            ...
            if args.task=='functionlearning':
                instructions = synthesize_functionlearning_problems('claude', version=f'v{prompt_version}', num_dim=num_dim, num_tasks=num_tasks)
                ...
                instructions = synthesize_decisionmaking_problems('claude', version=f'v{prompt_version}', num_dim=num_dim, num_tasks=num_tasks)
            else:
                raise NotImplementedError
        ...
        for run in range(first_run_id, first_run_id+num_runs):
            ...
            if len(matches)>0:
                for match in matches: 
                    targets.append(match[-1]) # last generated token are the targets
                    stimulus_dimensions.append(match[:-1]) # rest are feature names
                # import ipdb; ipdb.set_trace()
                # save data
                df = pd.DataFrame({'feature_names': stimulus_dimensions, 'targets': targets,  'task_id': np.arange(len(stimulus_dimensions)),})
