        ...
        for participant in range(start_participant, num_participants):
            ...
            for task in range(num_tasks):
                ...
                for block in range(start_block, num_blocks):
                    ...
                    df_block = df_task[(df_task['block'] == block)] if from_env else df_task[(
                        df_task['block'] == block)]
                    ...
                    if from_env:
                        ...
                        env = ShepardsTask(
                            task=block+1, return_prototype=True, batch_size=1, max_steps=96, shuffle_trials=True)
                        ...
                        feature_dict = {0: {0: 'Small', 1: 'Big'}, 1: {
                            0: 'Black', 1: 'White'}, 2: {0: 'Square', 1: 'Triangle'}}
                    ...
                    for t_idx, trial in enumerate(df_block.trial.values[:num_trials_block]):
                        ...
                        if from_env and (block >= 4):
                            t = str(
                                choice_options[targets[0][t_idx].numpy().astype(int)[0]])
                            # map the bar code input to the object feature in each dimension
                            barcode = packed_inputs[0][t_idx][:num_features].numpy().astype(
                                int)
                            object_name = ' '.join(
                                [feature_dict[i][barcode[i]] for i in range(len(barcode))])
                        else:
                        ...
                        df.loc[(df['participant'] == participant) & (df['task'] == task) & (
                            df['trial'] == trial) & (df['block'] == block), 'object'] = object_name
            ...
            df.to_csv(dataset.replace(
                '.csv', f'llm_choices{mode}_{start_participant}_shepard1961.csv'), index=False)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--from-env', action='store_true',
                        help='use environment or dataset')
    parser.add_argument('--num-runs', type=int, required=False,
                        default=1, help='number of runs')

    args = parser.parse_args()
    if args.dataset == 'badham2017':
        simulate_llm_on_shepard1961(mode=args.mode, model=args.model,
                                    start_participant=args.start_participant, from_env=args.from_env)
