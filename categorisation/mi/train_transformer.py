...
def run(env_name, restart_training, restart_episode_id, num_episodes, synthetic, nonlinear, rmc, num_dims, max_steps, sample_to_match_max_steps, noise, shuffle, shuffle_features, print_every, save_every, num_hidden, num_layers, d_model, num_head, save_dir, device, lr, batch_size=64):
    ...
    if synthetic:
        env = SyntheticCategorisationTask(nonlinear=nonlinear, num_dims=num_dims, max_steps=max_steps,
                                          batch_size=batch_size, noise=noise, shuffle_trials=shuffle, device=device).to(device)
    elif rmc:
        assert num_dims == 3 or num_dims == 4 or num_dims == 6, 'RMC supports 3, 4, and 6 dimensions'
        env = RMCTask(data=env_name, num_dims=num_dims, max_steps=max_steps,
                      batch_size=batch_size, noise=noise, shuffle_trials=shuffle, device=device).to(device)
    else:
        env = CategorisationTask(data=env_name, num_dims=num_dims, max_steps=max_steps, sample_to_match_max_steps=sample_to_match_max_steps,
    ...
    if restart_training and os.path.exists(save_dir):
        ...
        model = TransformerDecoder(num_input=env.num_dims, num_output=env.num_choices, num_hidden=num_hidden,
    ...
    for t in tqdm(range(start_id, int(num_episodes))):
        ...
        true_choices = targets.reshape(-1).float() if synthetic or rmc else torch.concat(
        ...
        if (not t % save_every):
            ...
            acc = evaluate_1d(env_name=env_name, model_path=save_dir, experiment=experiment, mode='val',
                              max_steps=max_steps, nonlinear=nonlinear, num_dims=num_dims, device=device)
            accuracy.append(acc)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description='meta-learning for categorisation')
    ...
    parser.add_argument(
        '--env-name', default='llama_generated_tasks_params65B_dim3_data8_tasks14500', help='name of the environment')
    parser.add_argument(
        '--env-dir', default='raven/u/ajagadish/vanilla-llama/categorisation/data', help='name of the environment')
    ...
    parser.add_argument('--rmc', action='store_true',
                        default=False, help='train models on rmc data')
    ...
    for i in range(args.runs):
        ...
        run(env_name, args.restart_training, args.restart_episode_id, args.num_episodes, args.synthetic, args.nonlinear, args.rmc, args.num_dims, args.max_steps, args.sample_to_match_max_steps,
