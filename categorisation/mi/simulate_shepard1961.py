...
def simulate_shepard1961(models=None, tasks=np.arange(1, 7), betas=None, num_runs=5, num_trials=96, num_blocks=1, batch_size=64):
    ...
    for m_idx, (model_name, beta) in enumerate(zip(models, betas)):
        if model_name != 'humans':
            for t_idx, task in enumerate(tasks):
                ...
                corrects[m_idx, t_idx] = evaluate_metalearner(
                    task, model_path, 'shepard_categorisation', beta=beta, shuffle_trials=True, num_trials=num_trials, num_runs=num_runs)
    ...
    for idx in np.arange(len(models)):
        for t_idx, rule in enumerate(data.keys()):
            block_errors = np.stack(
                np.split(errors[idx, t_idx], num_blocks)).mean(1)
...
def compute_mse(models):
    ...
    for beta in beta_range:
        betas = [None, beta]
        mse_distance_beta, mi_block_errors = simulate_shepard1961(
            models=models, betas=betas, num_runs=num_runs, num_blocks=num_blocks, num_trials=num_trials_per_block*num_blocks)
    ...
    # save results
    model_name = 'ermi' if 'claude' in models[1] else 'rmc' if 'rmc' in models[
        1] else 'pfn' if 'syntheticnonlinear' in models[1] else 'mi'
    np.save(f'{SYS_PATH}/categorisation/data/fitted_simulation/shepard1961_{model_name}_num_runs={num_runs}_num_blocks={num_blocks}_num_trials_per_block={num_trials_per_block}.npy',
            [np.array(mse_distances), beta_range])
