...
def fit_pm_to_humans(num_runs, num_blocks, num_iter, num_tasks, num_features, opt_method, loss, learn_prototypes, prototypes, task_name):
    ...
    for idx in range(num_runs):
        pm = PrototypeModel(num_features=NUM_FEATURES, distance_measure=1, num_iterations=num_iter,
                            learn_prototypes=learn_prototypes, prototypes=prototypes, loss=loss)
...
def fit_pm_to_metalearner(beta, num_runs, num_blocks, num_iter, num_tasks, num_features, opt_method, loss):
    ...
    lls, r2s, params_list = [], [], []
    for idx in range(num_runs):
        pm = PrototypeModel(num_features=NUM_FEATURES, distance_measure=1,
                            num_iterations=num_iter, learn_prototypes=False, prototypes='from_data', loss=loss)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='fit pm to meta-learner choices')
    parser.add_argument('--beta', type=float,
                        required=False, help='beta value')
    ...
    parser.add_argument('--learn-prototypes',
                        action='store_true', help='learn prototypes')
    parser.add_argument('--prototypes', type=str,
                        required=False, default=None, help='prototypes')
    parser.add_argument('--fit-human-data', action='store_true',
                        help='fit pm to human choices')
    parser.add_argument('--fit-llm', action='store_true',
                        help='fit pm to llm choices')
    ...
    if args.fit_human_data:
        fit_pm_to_humans(num_runs=args.num_runs, num_blocks=args.num_blocks, num_iter=args.num_iter, num_tasks=args.num_tasks, num_features=args.num_features,
                         opt_method=args.opt_method, loss=args.loss, learn_prototypes=args.learn_prototypes, prototypes=args.prototypes, task_name=args.task_name)

    elif args.fit_llm:
        fit_pm_to_llm(num_runs=args.num_runs, num_blocks=args.num_blocks, num_iter=args.num_iter, num_tasks=args.num_tasks, num_features=args.num_features, opt_method=args.opt_method,
                      loss=args.loss, learn_prototypes=args.learn_prototypes, prototypes=args.prototypes, task_name=args.task_name, model_name=args.model_name, method=args.method)
    else:
