...
def fit_rulex_to_humans(num_runs, num_blocks, num_iter, opt_method, loss, exception, task_name):
    ...
    if task_name == 'devraj2022':
        ...
        # TODO: Passing exceptions correctly
    ...
    for idx in range(num_runs):
        rulex = RulExModel(num_features=NUM_FEATURES, opt_method=opt_method,
                           loss=loss, exception=exception, exceptions=exceptions)
        ll, r2, params = rulex.fit_participants(
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='fit rulex to meta-learner or human choices')
    parser.add_argument('--exception', action='store_true',
                        help='use exception term')
