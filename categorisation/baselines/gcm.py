...
class GeneralizedContextModel():
    ...
    def __init__(self, num_features=4, num_categories=2, distance_measure=1, num_iterations=1, burn_in=False, opt_method='minimize', loss='nll'):
        ...
        self.bounds = [(0, 20.),  # sensitivity
                       (0, 1),  # epsilon greedy parameter
                       (0, 1),  # bias
                       ]
        self.num_defaults = len(self.bounds)
    ...
    def fit_participants(self, df, num_blocks=1, reduce='sum'):
        ...
        fit_measure, r2 = np.zeros((num_participants, num_conditions, num_blocks)), np.zeros(
            (num_participants, num_conditions, num_blocks))
        self.bounds.extend(self.weight_bound * self.num_features)
        store_params = np.zeros((num_participants, num_conditions, num_blocks, len(
            self.bounds)))  # store params for each task feature and block
        ...
        for p_idx, participant_id in enumerate(df['participant'].unique()[:num_participants]):
            ...
            for c_idx, condition_id in enumerate(df['condition'].unique()):
                df_condition = df_participant[(
                    df_participant['condition'] == condition_id)]
                ...
                for b_idx, block in enumerate(range(num_blocks)):
                    ...
                    df_condition_block = df_condition[(df_condition['trial'] < (
                        (block+1)*num_trials_per_block + offset_trial)) & (df_condition['trial'] >= (block*num_trials_per_block + offset_trial))]
                    best_params = self.fit_parameters(
                        df_condition_block, reduce)
                    fit_measure[p_idx, c_idx, b_idx] = self.loss_fn(
                        best_params, df_condition_block, reduce)
                    if self.loss == 'nll':
                        num_trials = len(df_condition_block) * \
                            (df_condition_block.task.max()+1)
                        num_trials = num_trials*0.5 if self.burn_in else num_trials
                        r2[p_idx, c_idx, b_idx] = 1 - \
                            (fit_measure[p_idx, c_idx, b_idx] /
                             (-num_trials*np.log(1/2)))
    ...
    def fit_metalearner(self, df, num_blocks=1, reduce='sum'):
        ...
        fit_measure, r2 = np.zeros((num_runs, num_task_features, num_blocks)), np.zeros(
            (num_runs, num_task_features, num_blocks))
        self.bounds.extend(self.weight_bound * self.num_features)
        store_params = np.zeros(
            (num_runs, num_task_features, num_blocks, len(self.bounds)))
        ...
        for r_idx, run_id in enumerate(df['run'].unique()[:num_runs]):
            ...
            for idx, task_feature_id in enumerate(df_run['task_feature'].unique()):
                df_task_feature = df_run[(
                    df_run['task_feature'] == task_feature_id)]
                num_trials_per_block = int(
                    (df_task_feature.trial.max()+1)/num_blocks)
                for b_idx, block in enumerate(range(num_blocks)):
                    df_task_feature_block = df_task_feature[(df_task_feature['trial'] < (
                        block+1)*num_trials_per_block) & (df_task_feature['trial'] >= block*num_trials_per_block)]
                    best_params = self.fit_parameters(
                        df_task_feature_block, reduce)
                    fit_measure[r_idx, idx, b_idx] = self.loss_fn(
                        best_params, df_task_feature_block, reduce)
                    # fit_measure[idx, b_idx] = -self.compute_nll(best_params, df_task_feature_block, reduce)
                    if self.loss == 'nll':
                        num_trials = len(df_task_feature_block) * \
                            (df_task_feature_block.task.max()+1)
                        num_trials = num_trials*0.5 if self.burn_in else num_trials
                        r2[r_idx, idx, b_idx] = 1 - \
                            (fit_measure[r_idx, idx, b_idx] /
                             (-num_trials*np.log(1/2)))
    ...
    def fit_llm(self, df, num_blocks=1, reduce='sum'):
        """ fit gcm to llm choices and compute negative log likelihood 
        ...
        for p_idx, participant_id in enumerate(df['participant'].unique()[:num_participants]):
            ...
            df_participant['choice'] = df_participant['llm_category'].map(
                mapping)
            df_participant['correct_choice'] = df_participant['true_category'].map(
                mapping)
            df_participant['category'] = df_participant['true_category'].map(
                mapping)
    ...
    def constraint(self, params):
        ...
        return np.sum(params[self.num_defaults:]) - 1
    ...
    def compute_nll(self, params, df, reduce):
        ...
        for task_id in range(num_tasks):
            ...
            # list of lists to store objects seen so far within each category
            stimuli_seen = [[] for i in range(self.num_categories)]
            for trial_id in df_task['trial'].values:
                df_trial = df_task[(df_task['trial'] == trial_id)]
                choice = categories[df_trial.choice.item()] if df_trial.choice.item(
                ) in categories else df_trial.choice.item()
                true_choice = categories[df_trial.correct_choice.item()] if df_trial.correct_choice.item(
                ) in categories else df_trial.correct_choice.item()

                # load num features of the current stimuli
                current_stimuli = df_trial[['feature{}'.format(
                    i+1) for i in range(self.num_features)]].values

                # given stimuli and list of objects seen so far within cateogry return probablity the object belongs to each category
                category_probabilities = self.gcm(
                    params, current_stimuli, stimuli_seen, choice, reduce=reduce)
                p_choice = category_probabilities[choice] * \
                    (1-params[1]) + params[1]*(1/self.num_categories)
                if self.burn_in:
                    ll += 0 if (trial_id < int(num_trials/2)
                                ) else np.log(p_choice + epsilon)
                else:
    ...
    def compute_nll_transfer(self, params, df_train, df_transfer):
        ...
        ll = 0.
        stimuli_seen = [df_train[df_train['category'] == i][['x{}'.format(
            i+1) for i in range(self.num_features)]].values for i in range(self.num_categories)]
        stimuli_seen = [np.expand_dims(stimuli_seen[i], axis=1)
        ...
        for trial_id in df_transfer.trial_id.values:
            ...
            current_stimuli = df_trial[['x{}'.format(
                i+1) for i in range(self.num_features)]].values
    ...
    def compute_mse_transfer(self, params, df_train, reduce):
        ...
        stimuli_seen = [df_train[df_train['category'] == category][['feature{}'.format(
            i+1) for i in range(self.num_features)]].values for category in np.sort(df_train['category'].unique())]
        stimuli_seen = [np.expand_dims(stimuli_seen[i], axis=1)
                        for i in range(self.num_categories)]
        REF_CATEGORY = 1
        # keep one instance of each stimulus in order of stimulus_id
        df_transfer = df_train.drop_duplicates(
            subset=['stimulus_id'], keep='first').sort_values(by=['stimulus_id'])
        ...
        proportion_category_1 = np.array([np.mean(df_train[df_train['stimulus_id'] == i]
                                         ['choice'].values == REF_CATEGORY) for i in df_transfer['stimulus_id'].values])
        ...
        for idx, stimulus_id in enumerate(df_transfer.stimulus_id.values):
            ...
            current_stimuli = df_trial[['feature{}'.format(
                i+1) for i in range(self.num_features)]].values
            category_probabilities = self.gcm(
                params, current_stimuli, stimuli_seen, None, reduce=reduce)
    ...
    def benchmark(self, df_train, df_transfer):
        ...
        log_likelihood = - \
            self.compute_nll_transfer(result.x, df_train, df_transfer)/2
    ...
    def gcm(self, params, current_stimuli, stimuli_seen, choice, reduce='sum'):
        ...
        sensitivity, _, bias = params[:self.num_defaults]
        weights = params[self.num_defaults:]
        ...
        for for_category in range(self.num_categories):
            if len(stimuli_seen[for_category]) == 0:
                ...
                category_similarity[for_category] = self.compute_attention_weighted_similarity(
                    current_stimuli, np.stack(stimuli_seen[for_category]), (weights, sensitivity), reduce=reduce)

        # compute category probabilities
        category_probabilities = self.compute_category_probabilities(
            category_similarity, bias)
    ...
    def compute_attention_weighted_similarity(self, x, y, params, reduce='sum'):
        ...
        d = weights.reshape((1, -1)) @ (np.abs(y.squeeze(1)-x)
                                        ** self.distance_measure).T
