...
def process_participant(participant_data):
    ...
    ll, r2, params = gcm.fit_participants(participant_data, num_blocks=num_blocks, reduce='sum')
    return ll, r2, params
...
lls = np.array(lls)
r2s = np.array(r2s)
np.savez(f'../data/meta_learner/parallel_gcm_humans_devrajstask_runs={num_runs}_iters={num_iter}_blocks={num_blocks}_tasks={NUM_TASKS}'\
